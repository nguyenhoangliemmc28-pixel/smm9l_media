...
export function Card({ children, hover, gradient, glow, className='', ...rest }: ICardProps) {
  return <motion.div whileHover={hover ? { y: -6 } : undefined} transition={{ duration: 0.25, ease:'easeOut' }} className={`relative rounded-card ${gradient ? 'gradient-border' : 'glass'} ${glow ? 'shadow-glow' : 'shadow-card'} ${hover ? 'hover:border-border-strong cursor-pointer' : ''} ${className}`} {...rest}>{children}</motion.div>;
}
