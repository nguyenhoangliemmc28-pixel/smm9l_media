...
export async function createOrder(
  ...
  // Forward to provider API via edge function (fire and forget)
  const orderId = result.order_id;
  if (orderId) {
    forwardOrderToProvider(serviceId, link, quantity, orderId);
  }

  return { success: true, orderId, message: result.message };
}

async function forwardOrderToProvider(serviceId: string, link: string, quantity: number, orderId: string): Promise<void> {
  try {
    ...
    const edgeUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/provider-api`;
    await fetch(edgeUrl, {
      method: 'POST',
      headers: {
    ...
    // Provider forwarding failed - order stays PENDING, admin can manually sync
  }
}
...
export async function fetchWalletStats(): Promise<{
  deposited: number;
  spent: number;
  refunded: number;
  commission: number;
...
export async function createDeposit(
  bank: string,
  amount: number,
): Promise<{ success: boolean; depositId?: string; txnCode?: string; message?: string }> {
  const { data: userData } = await supabase.auth.getUser();
  const user = userData.user;
  if (!user) return { success: false, message: 'Chưa đăng nhập' };
  const txnCode = 'DEP' + Date.now().toString().slice(-8);
  const { data, error } = await supabase
    .from('deposits')
    .insert({ user_id: user.id, txn_code: txnCode, bank, amount, status: 'PENDING' })
    .select('id')
    .single();
  if (error) return { success: false, message: error.message };
  // Auto-confirm via RPC (simulating bank webhook)
  setTimeout(async () => {
    await supabase.rpc('process_deposit', { p_deposit_id: data.id });
  }, 3000);
  return { success: true, depositId: data.id, txnCode };
}
...
export async function createTicket(
  ...
  const { data, error } = await supabase
    .from('tickets')
    .insert({ user_id: user.id, subject, department, priority: 'MEDIUM', status: 'OPEN' })
    .select('id')
    .single();
  if (error) return { success: false, message: error.message };
  await supabase.from('ticket_replies').insert({ ticket_id: data.id, user_id: user.id, message });
...
export async function sendTicketReply(ticketId: string, message: string): Promise<void> {
  ...
  const user = userData.user;
  if (!user) return;
  await supabase.from('ticket_replies').insert({ ticket_id: ticketId, user_id: user.id, message });
  await supabase.from('tickets').update({ status: 'USER_REPLY' }).eq('id', ticketId);
}
...
export async function generateApiKey(): Promise<string> {
  ...
  const { error } = await supabase.from('api_keys').insert({ user_id: user.id, key, status: 'ACTIVE' });
  if (error) throw error;
  return key;
}
...
export async function fetchDashboardHomeStats(): Promise<{
  open_tickets: number;
  commission: number;
  total_orders: number;
...
export async function fetchPublicSettings(): Promise<Record<string, unknown>> {
  const { data, error } = await supabase
    .from('settings')
    .select('key, value')
    .in('key', ['bank_name', 'bank_account', 'bank_holder', 'deposit_min', 'deposit_max', 'withdraw_min', 'withdraw_fee', 'site_name']);
  if (error) return {};
...
// ============ Admin (uses SECURITY DEFINER RPCs with role check) ============
export async function fetchAllUsers(): Promise<{
  id: string; username: string; email: string; balance: number; role: string; status: string; created_at: string;
}[]> {
