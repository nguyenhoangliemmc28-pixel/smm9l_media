...
import { Plus, Pencil, Trash2, Server, Wifi, Download, RefreshCw, Activity, ShieldCheck, Eye, EyeOff, ExternalLink, DollarSign } from 'lucide-react';
...
export function AdminProvidersPage() {
  ...
  const [modalOpen, setModalOpen] = useState(false); const [editing, setEditing] = useState<IProvider | null>(null);
  const [form, setForm] = useState(emptyForm); const [saving, setSaving] = useState(false); const [deleteId, setDeleteId] = useState<string | null>(null);
  const [testing, setTesting] = useState<string | null>(null); const [importing, setImporting] = useState<string | null>(null); const [syncing, setSyncing] = useState<string | null>(null); const [showKey, setShowKey] = useState<string | null>(null);
  ...
  const handleSave = async () => {
    if (!form.name || !form.api_url || !form.api_key) { toast('Vui lòng nhập đủ thông tin provider', 'error'); return; }
    setSaving(true); try { if (editing) await updateProvider(editing.id, form.name, form.api_url, form.api_key, form.status, form.description); else await createProvider(form.name, form.api_url, form.api_key, form.description); toast(editing ? 'Đã cập nhật provider' : 'Đã tạo provider', 'success'); setModalOpen(false); refetch(); } catch (e: any) { toast(e.message ?? 'Lỗi', 'error'); } finally { setSaving(false); }
  };
  const handleDelete = async (id: string) => { try { await deleteProvider(id); toast('Đã xóa provider', 'success'); refetch(); } catch (e: any) { toast(e.message ?? 'Lỗi', 'error'); } };
  const testConnection = async (p: IProvider) => { setTesting(p.id); try { const r = await testProviderConnection(p.id); r.success ? toast(r.message ?? `Kết nối ${p.name} thành công`, 'success') : toast(r.message ?? 'Kết nối thất bại', 'error'); } catch (e: any) { toast(e.message ?? 'Không thể kết nối', 'error'); } finally { setTesting(null); } };
  const handleImport = async (p: IProvider) => { setImporting(p.id); try { const r = await importProviderServices(p.id); r.success ? toast(`Đã import ${r.imported ?? 0} dịch vụ`, 'success') : toast(r.message ?? 'Import thất bại', 'error'); } catch (e: any) { toast(e.message ?? 'Lỗi', 'error'); } finally { setImporting(null); } };
  const handleSyncPrices = async (p: IProvider) => { setSyncing(p.id); try { const r = await syncProviderPrices(p.id); r.success ? toast(`Đã đồng bộ ${r.synced ?? 0} giá dịch vụ`, 'success') : toast(r.message ?? 'Đồng bộ thất bại', 'error'); } catch (e: any) { toast(e.message ?? 'Lỗi', 'error'); } finally { setSyncing(null); } };
    ...
    {loading ? <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">{Array.from({length:6}).map((_,i)=><div key={i} className="skeleton h-64 rounded-card"/>)}</div> : list.length===0 ? <Card><EmptyState icon={Server} title="Chưa có provider" description="Thêm nhà cung cấp API đầu tiên để bắt đầu đồng bộ dịch vụ." /></Card> : <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
      {list.map((p,i)=><motion.div key={p.id} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:i*.04}} whileHover={{y:-3}}>
        <Card hover className="p-5 h-full flex flex-col overflow-hidden relative">
          ...
          <div className="relative flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="h-11 w-11 rounded-xl bg-gradient-to-br from-primary-500/20 to-secondary-500/10 border border-primary-500/15 flex items-center justify-center"><Server className="h-5 w-5 text-primary-300"/></div><div><h3 className="text-sm font-semibold text-white">{p.name}</h3><Badge tone={p.status==='ACTIVE'?'success':'neutral'} size="sm" dot>{p.status==='ACTIVE'?'ONLINE':'OFFLINE'}</Badge></div></div><div className="flex gap-1"><button onClick={()=>openEdit(p)} className="h-8 w-8 rounded-lg flex items-center justify-center text-text-muted hover:text-white hover:bg-white/[.06]"><Pencil className="h-4 w-4"/></button><button onClick={()=>setDeleteId(p.id)} className="h-8 w-8 rounded-lg flex items-center justify-center text-text-muted hover:text-danger hover:bg-danger/10"><Trash2 className="h-4 w-4"/></button></div></div>
          <div className="mt-4 rounded-xl border border-border bg-bg-soft/40 p-3 space-y-2.5"><div className="flex items-center gap-2 text-xs"><span className="text-text-dim w-12">URL</span><code className="truncate flex-1 text-text-muted">{p.api_url}</code><a href={p.api_url} target="_blank" rel="noreferrer" className="text-text-dim hover:text-primary-300"><ExternalLink className="h-3.5 w-3.5"/></a></div><div className="flex items-center gap-2 text-xs"><span className="text-text-dim w-12">KEY</span><code className="truncate flex-1 text-text-muted">{showKey===p.id?p.api_key:`${p.api_key.slice(0,6)}••••••`}</code><button onClick={()=>setShowKey(showKey===p.id?null:p.id)} className="text-text-dim hover:text-white">{showKey===p.id?<EyeOff className="h-3.5 w-3.5"/>:<Eye className="h-3.5 w-3.5"/>}</button></div></div>
