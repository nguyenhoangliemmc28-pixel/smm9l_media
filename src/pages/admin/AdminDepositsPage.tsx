...
import { fetchAllDeposits, rejectDeposit, fetchWithdrawals, processWithdrawal } from '@/lib/admin';
...
export function AdminDepositsPage() {
  ...
  const handleApprove = async (id: string) => {
    setActionLoading(id);
    try {
      const { data, error } = await supabase.rpc('admin_process_deposit', { p_deposit_id: id, p_approve: true, p_note: 'Duyệt từ Admin Dashboard' });
      if (error) throw error;
      const result = data as { success?: boolean; message?: string } | null;
      if (result?.success === false) throw new Error(result.message ?? 'Không thể duyệt nạp tiền');
      toast('Đã duyệt nạp tiền', 'success');
      refetch();
    } catch (e: any) {
            ...
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {paginated.map((d, i) => (
                      <tr key={d.id} className="border-b border-border/40 hover:bg-white/[0.02]">
          ...
          {(withdrawals ?? []).length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(withdrawals ?? []).map((w, i) => (
                    <tr key={w.id} className="border-b border-border/40 hover:bg-white/[0.02]">
