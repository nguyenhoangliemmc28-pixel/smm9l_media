...
import { Copy, Check, Landmark, Sparkles, ShieldCheck, Wallet, ArrowDownToLine } from 'lucide-react';
...
export function DepositPage() {
  ...
  const depositMax = Number(settings?.deposit_max ?? 999999999);
  ...
  const submit = async () => {
    if (!Number.isFinite(amount) || amount < depositMin || amount > depositMax) {
      return toast(`Số tiền phải từ ${formatCurrency(depositMin)} đến ${formatCurrency(depositMax)}`, 'error');
    }
    setSubmitting(true);
    const result = await createDeposit(bankName, amount);
    setSubmitting(false);
    if (result.success) {
      toast('Yêu cầu nạp tiền đã được tạo và đang chờ đối soát.', 'success');
      refetch();
    } else {
  ...
  return <div className="space-y-6">
    <div className="relative overflow-hidden rounded-2xl border border-primary-500/20 bg-gradient-to-br from-primary-500/12 via-bg-card to-secondary-500/10 p-5 sm:p-7">
      ...
      <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-5">
        <div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-primary-300 mb-2"><Sparkles className="h-3.5 w-3.5"/> 9L Media Wallet</div><h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-white">Nạp tiền</h1><p className="text-sm text-text-muted mt-1.5">Nạp qua chuyển khoản ngân hàng và chờ admin đối soát trước khi cộng số dư.</p></div>
