...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: profile } = await supabase
      ...
      .eq("id", userId)
      .maybeSingle();
    const isAdmin = ["ADMIN", "SUPER_ADMIN", "admin", "super_admin"].includes(profile?.role ?? "");
    ...
    switch (action) {
      ...
      case "place_order": {
        ...
        if (!serviceId || !link || !Number.isInteger(quantity) || quantity <= 0 || !orderId) {
          return json({ success: false, message: "Invalid order payload" }, 400);
        }

        const { data: order } = await supabase
          .from("orders")
          .select("id, user_id, service_id, provider_order_id, status")
          .eq("id", orderId)
          .eq("user_id", userId)
        ...
        const { data: service } = await supabase
          ...
          .eq("id", serviceId)
          .single();
        if (!service || service.provider_id !== providerId || !service.provider_service_id) {
          return json({ success: false, message: "Service/provider mapping is invalid" }, 400);
        }
      ...
      case "sync_order": {
        ...
        const { data: order } = await supabase
          .from("orders")
          .select("id, user_id, service_id, provider_order_id")
          .eq("id", orderId)
          .eq("user_id", userId)
          .maybeSingle();
